...
class Gbx {
    ...
    constructor(server, options = {}) {
        ...
        this.socket = null;
        this.recvData = node_buffer_1.Buffer.from([]);
    ...
    async handleData(data) {
        ...
        if (data) {
            this.recvData = node_buffer_1.Buffer.concat([this.recvData, data]);
        }
        ...
        while (true) {
            ...
            if (this.responseLength &&
                ...
                if (!this.isConnected) {
                    ...
                    const readable = stream_1.Readable.from(message.subarray(4));
                    if (requestHandle >= 0x80000000) {
                        const cb = this.promiseCallbacks[requestHandle];
                        if (cb) {
                            deserializer.deserializeMethodResponse(readable, async (err, res) => {
                                cb.resolve([res, err]);
                        ...
                        deserializer.deserializeMethodCall(readable, async (err, method, res) => {
    ...
    async multicall(methods) {
        ...
        const xml = serializer_1.default.serializeMethodCall("system.multicall", [
            params,
        ]);
        const out = [];
    ...
    async query(xml, wait = true) {
        ...
        const len = node_buffer_1.Buffer.byteLength(xml, "utf-8");
        const buf = node_buffer_1.Buffer.alloc(HEADER_LENGTH + len);
